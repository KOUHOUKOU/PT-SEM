...
import numpy as np
import pandas as pd
from scipy.special import gammaincc, gammaln, hyp1f1, logsumexp, xlogy
...
def convolution_loglik(
    ...
    Geometric uses the Poisson-CDF closed form.  NB uses the terminating
    confluent-hypergeometric representation of the Delaporte distribution.
    ...
    if family == "NB":
        ...
        q = 1.0 - p
        if size >= 1e6:
            # Moment inversion uses a large size at the Poisson boundary.
            epsilon_mean = size * q / p
            return float(poisson_logpmf_vector(thinning_mean + epsilon_mean).sum())
        base = (
            -thinning_mean
            + gammaln(observations + size)
            - gammaln(size)
            - log_factorial
            + size * math.log(p)
            + observations * math.log(q)
        ...
        hyper = hyp1f1(
            ...
            1.0 - size - observations,
            thinning_mean / q,
        )
        # scipy may report a pole for x=0 when size is integer, although the
        # terminating polynomial is exactly one in that case.
        hyper = np.asarray(hyper, dtype=float)
        hyper[observations == 0] = 1.0
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            values = base + np.log(hyper)
        invalid = ~np.isfinite(values)
        if invalid.any():
            for index in np.flatnonzero(invalid):
                values[index] = _single_convolution_logpmf(
