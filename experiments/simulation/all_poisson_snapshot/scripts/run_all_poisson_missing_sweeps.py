#!/usr/bin/env python3
"""Run missing JMLR All-Poisson cells through the formal M-setting framework."""
...
import all_poisson_formal_framework as formal
...
CONFIG_PATH = (
    ...
    / "frozen_configs"
    / "all_poisson"
    / "jmlr_all_poisson_formal_design_v2.json"
)
...
REUSED_RESTRICTED = RESULT_ROOT / "raw" / "reused_restricted_sample_size.csv"
...
def stable_cell_base_seed(master_seed: int, sweep: str, value: object) -> int:
    payload = (
        f"{master_seed}|all_poisson|{sweep}|{canonical_value(value)}"
    ).encode("utf-8")
...
def load_and_validate_config() -> dict[str, object]:
    ...
    if config["version"] != "jmlr_all_poisson_formal_framework_v2":
    ...
    for regime in ("restricted", "extended"):
        ...
        for sweep in ("dimension", "sample_size", "average_in_degree"):
            ...
            for value in sweep_config["values"]:
                if regime == "restricted" and sweep == "sample_size" and bool(sweep_config.get("reuse_restricted")):
                    continue
                if sweep == "dimension":
                    dimension = int(value)
...
def cell_directory(cell: dict[str, object]) -> Path:
    return (
        ...
        / str(cell["regime"])
        / str(cell["sweep"])
        / f"value_{path_value(cell['sweep_value'])}"
    )
...
def seed_plan(config: dict[str, object], cells: list[dict[str, object]], reps: int) -> pd.DataFrame:
    ...
    for cell in cells:
        for replication in range(reps):
            ...
            rows.append(
                {
                    "source": "formal_missing_experiment",
                    "regime": cell["regime"],
                    "sweep": cell["sweep"],
                    ...
                    "cell_base_seed": base,
                    "data_seed": formal.d.derive_seed(base, int(cell["d"]), replication, stream=0),
                    "paired_across_regimes": True,
                }
            )
    if REUSED_RESTRICTED.exists():
        reused = pd.read_csv(REUSED_RESTRICTED).drop_duplicates(["sweep_value", "replication"])
        for row in reused.itertuples(index=False):
            rows.append(
                {
                    "source": "reused_historical_restricted_sample_size",
                    "regime": "restricted",
                    "sweep": "sample_size",
                    "sweep_value": row.sweep_value,
                    "d": row.d,
                    "N": row.N,
                    "average_in_degree": row.average_in_degree,
                    "replication": row.replication,
                    "cell_base_seed": "historical_sha256_rule",
                    "data_seed": row.seed,
                    "paired_across_regimes": False,
                }
            )
    frame = pd.DataFrame(rows).sort_values(
        ["regime", "sweep", "sweep_value", "replication"], kind="stable"
    )
...
def main() -> None:
    ...
    reps = int(args.R or config["replications"])
    if workers < 1 or reps < 1:
        raise SystemExit("--workers and --R must be positive")
    ...
    plan = {
        ...
        "R": reps,
        "cells": cells,
        "new_replications": len(cells) * reps,
        "restricted_sample_size_reused": bool(REUSED_RESTRICTED.exists()),
    }
    atomic_json(plan, METADATA_ROOT / "formal_run_plan.json")
    print(json.dumps({"cells": len(cells), "new_replications": len(cells) * reps, "workers": workers}, indent=2), flush=True)
    if args.plan_only:
        return
    ...
    if args.preflight_only:
        return
    selected = cells[: args.max_cells] if args.max_cells is not None else cells
    for index, cell in enumerate(selected, start=1):
        print(
            f"[{index}/{len(selected)}] {cell['regime']} {cell['sweep']}={cell['sweep_value']} "
            f"d={cell['d']} N={cell['N']} k={cell['average_indegree']}",
            flush=True,
        )
        run_cell(cell, reps, workers)
    print("All selected formal All-Poisson cells passed strict validation.", flush=True)
