#!/usr/bin/env python3
"""Audited All-Poisson specialization of the frozen paper-final framework.

The frozen implementation is imported from its original snapshot location so
its external-baseline paths, method implementations, metrics, multiprocessing,
checkpointing, and summary code remain unchanged.  The sole DGP specialization
...
editing anything under ``frozen_code``.
"""
...
# not alter the frozen method or author source.
...
FROZEN_METHODS = SNAPSHOT / "frozen_code" / "methods"
FROZEN_D = FROZEN_METHODS / "d.py"
...
sys.path.insert(0, str(FROZEN_METHODS))

import d  # noqa: E402


if Path(d.__file__).resolve() != FROZEN_D.resolve():
    raise RuntimeError(
        f"Imported unexpected d.py: {Path(d.__file__).resolve()} != {FROZEN_D.resolve()}"
    )
...
def install_all_poisson_specialization() -> None:
    """Install the minimal specialization into the frozen module namespace."""
...
def source_sha256() -> str:
    ...
    with FROZEN_D.open("rb") as handle:
...
def framework_provenance() -> dict[str, object]:
    return {
        "frozen_d_path": str(FROZEN_D),
        "frozen_d_sha256": source_sha256(),
