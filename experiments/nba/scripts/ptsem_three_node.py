...
def fit_local(data, child, parent_mask, family, maxiter):
    ...
    if family == "poisson":
        ...
        bounds = alpha_bounds + [(1e-8, max(float(y.max()) * 5.0 + 10.0, 10.0)), (1e-6, 1e8)]
...
def build_local_scores(data, family, maxiter=300, workers=1):
    d = data.shape[1]
    tasks = []
    ...
    if workers <= 1:
        for result in map(_fit_local_task, tasks):
            child, parent_mask, bic, fit = result
            local[child][parent_mask] = bic
            fits[child][parent_mask] = fit
    else:
        with ProcessPoolExecutor(max_workers=workers) as pool:
            futures = [pool.submit(_fit_local_task, task) for task in tasks]
...
def enumerate_dags(local, top_k=10):
    d = len(local)
