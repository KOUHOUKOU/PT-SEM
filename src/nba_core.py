...
MODEL_FAMILY_METADATA: Dict[str, FamilyMetadata] = {
    ...
    "Binomial": FamilyMetadata(
        1,
        "integers 0,...,n in a caller-declared fixed finite set (default {2,...,50})",
    ),
...
def _validated_family_params(
    family: str,
    params: Dict[str, float],
    binomial_n_max: int = ESTIMATOR_BINOMIAL_N_MAX,
) -> Dict[str, float]:
    """Validate parameters without silently clipping to another model."""
    if (
        not isinstance(binomial_n_max, int)
        or binomial_n_max < ESTIMATOR_BINOMIAL_N_MIN
    ):
        raise ValueError("binomial_n_max must be an integer >= 2")
    if family == "Poisson":
    ...
    if family in ("Binomial", "Bernoulli"):
        ...
        if family == "Binomial" and not (
            ESTIMATOR_BINOMIAL_N_MIN <= trials <= binomial_n_max
        ):
            raise ValueError(
                "Binomial requires integer n in "
                f"{{{ESTIMATOR_BINOMIAL_N_MIN},...,{binomial_n_max}}}"
            )
        p = float(params["p"])
        if not math.isfinite(p) or not 0.0 < p < 1.0:
...
def eps_logpmf(
    ...
    params = _validated_family_params(family, params, binomial_n_max)
    if family == "Poisson":
        lam = float(params["lam"])
...
def binomial_moment_map(
    ...
    into the explicitly selected fixed estimator space before p=m/n is
    evaluated.  Simulation uses the default {2,...,50}; a real-data caller
    may declare another fixed, truth-independent finite maximum.
...
def _moment_map_with_extension(
    ...
    if family == "Binomial":
        return binomial_moment_map(mean, variance, binomial_n_max)
...
def invert_moments(
    ...
    params, _ = _moment_map_with_extension(
        family, mean, variance, binomial_n_max
    )
